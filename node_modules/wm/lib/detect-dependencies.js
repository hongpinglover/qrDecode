...
var findComponentConfigFile = function (config, component) {
  ...
  ['.bower.json',
    ...
    if (!_.isObject(componentConfigFile) && fs.existsSync(configFile)) {
      ...
      //console.log(configFile , JSON.stringify(componentConfigFile));
    }
  });
...
var filterByComp = function(filterComps , globalDeps ,  dependencies){
     /*
     for(var i=0 ; i<filterComps.length; i++){
         var _dep = globalDeps.get(filterComps[i]);
         var main = _dep.main;
...
module.exports = function detect(config) {
 ...
 var allDependencies = {};
  ...
  var jsdep = prioritizeDependencies(config, '.js');
  var cssdep = prioritizeDependencies(config, '.css');

  var filterComps  =  config.get("filterComps");
  if(typeof filterComps != 'undefined'){
     var globalDeps = config.get('global-dependencies');
     jsdep = filterByComp(filterComps , globalDeps , jsdep);
     cssdep = filterByComp(filterComps , globalDeps , cssdep);
  }
  
  var retDeps = filterExcludedDependencies({
    js: jsdep,
    css: cssdep,
  }, config.get('exclude'));
